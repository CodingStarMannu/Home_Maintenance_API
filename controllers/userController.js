...
const user_location = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      msg: "User location saved successfully.",
...
const getNearbyVendors = async (req, res) => {
  try {
    ...
    const allVendorLocations = await VendorLocation.find();

    console.log("allVendorLocations:", allVendorLocations);

    const nearbyVendors = allVendorLocations
      .map((vendorLocation) => {
        ...
        return { vendorLocation, distance };
      })
      .filter((item) => item.distance <= radius)
      .sort((a, b) => a.distance - b.distance)
      .slice(0, 10); // Limit to 10 nearby vendors

    res.status(200).json({ success: true, nearbyVendors });
  } catch (error) {
    console.error("Error in getting nearby vendors:", error);
    res.status(500).json({ success: false, message: "Internal Server Error" });
  }
};
